...
class ContentProcessor:
    ...
    def process_learning_object(self, file_content: bytes, file_name: str, metadata: Dict) -> Dict:
        """Procesa un objeto de aprendizaje y extrae sus componentes"""
        ...
        if file_extension == 'pdf':
            ...
            # Para otros tipos de archivo, por ahora usamos el nombre y metadata
            text_content = f"Título: {metadata.get('title', '')}\nAutor: {metadata.get('author', '')}"

        # Usar IA para extraer componentes
        components = self.ai_service.extract_components_from_text(text_content)
    ...
    def create_component_files(self, components: Dict, object_id: int, base_file_name: str) -> List[Dict]:
        ...
        if components.get("objetivos"):
            ...
            file_info = self.s3_service.upload_file(
                ...
                original_file_name=f"{base_file_name}_objetivos.pdf",  # ✅ PDF en lugar de TXT
        ...
        if components.get("teoria"):
            ...
            file_info = self.s3_service.upload_file(
                file_content=pdf_content,
                original_file_name=f"{base_file_name}_teoria.pdf",  # ✅ PDF en lugar de TXT
                object_id=str(object_id),
        ...
        if components.get("ejercicios"):
            ...
            file_info = self.s3_service.upload_file(
                file_content=pdf_content,
                original_file_name=f"{base_file_name}_ejercicios.pdf",  # ✅ PDF en lugar de TXT
                object_id=str(object_id),
